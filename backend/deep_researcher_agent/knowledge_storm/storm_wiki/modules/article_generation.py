...
class QueryRewrite(dspy.Signature):
    ...
    queries = dspy.InputField(description="List of search queries to rewrite", format=list)
    rewritten_queries = dspy.OutputField(description="Rewritten search queries, one per line, without any prefix or suffix:")
...
def filter_queries(raw_output):
    ...
    for i, (orig_line, lower_line) in enumerate(zip(original_lines, lowercase_lines)):
        ...
        if ':' in lower_stripped and "rewritten queries" in lower_stripped.split(':', 1)[0]:
...
class StormArticleGenerationModule(ArticleGenerationModule):
    ...
    def generate_section(
        ...
        with dspy.settings.context(lm=self.article_gen_lm):
            rewritten_queries_str = self.query_rewrite(queries=section_query).rewritten_queries
        ...
        # Log results
        logging.info(f"Retrieved {len(collected_info)} information objects for section {section_name}")
    ...
    def generate_article(
        ...
        if article_with_outline is None:
            ...
            logging.warning("No outline provided to generate_article. Proceeding with an empty structure.")
        ...
        def get_sections_with_leaf_subheadings(node, parent_headings=None):
            ...
            if not isinstance(node.section_name, str):
                 logging.error(f"Node section_name is not a string: {node.section_name}. Skipping node.")
                 return section_data

            current_headings = parent_headings + [node.section_name]
            subheadings = [child.section_name for child in node.children if isinstance(child.section_name, str)] # Filter non-string names
            # is_leaf = len(subheadings) == 0 

            # Check if the current node has leaf children
            leaf_children = [child for child in node.children if len(child.children) == 0 and isinstance(child.section_name, str)]
            if leaf_children: # Check if there are any leaf children
                section_data.append({
                    ...
                    "leaf_subheadings": [child.section_name for child in leaf_children], # Get names of leaf children
                })

            # Recurse into non-leaf children
            for child in node.children:
                 if len(child.children) > 0: # Only recurse if the child is not a leaf
                      section_data.extend(get_sections_with_leaf_subheadings(child, current_headings))
            return section_data
        ...
        for child in article_with_outline.root.children: # Starts at Level 1
            all_potential_sections.extend(get_sections_with_leaf_subheadings(child, [article_with_outline.root.section_name])) # Pass root name as initial parent
            ...
            for child in article_with_outline.root.children:
                if child.children and isinstance(child.section_name, str):
                    sections_to_process.append({
                        ...
                        "leaf_subheadings": [c.section_name for c in child.children if isinstance(c.section_name, str)]
                    })
        else:
            # Original logic for 3-level outlines - process sections at Level 2 or deeper with leaf children
            sections_to_process = [
                section_data for section_data in all_potential_sections
                if len(section_data["parent_headings"]) >= 2  # Check if depth is >= 2 (root is level 0)
            ]

        if not sections_to_process:
             logging.warning("No sections identified for content generation based on the outline structure and leaf node criteria.")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_thread_num) as executor:
            ...
            for section_data in sections_to_process:
                ...
                section_node = article_with_outline.find_section(article_with_outline.root, section_title)
                ...
                if len(parent_headings) == 1:  # Level 1 section (root is level 0)
                    ...
                    section_query.extend(get_all_hierarchy_headings(section_node, include_self=False))
                else:
                ...
                current_level = len(parent_headings) # Level relative to root (root is level 0)
                queries_with_hashtags = [f"{'#' * current_level} {section_title}"]
                for subheading in leaf_subheadings:
                    queries_with_hashtags.append(f"{'#' * (current_level + 1)} {subheading}")
            ...
            for future in as_completed(future_to_section):
                try:
                    ...
                    parent_section_name = section_data["parent_headings"][-1] if section_data["parent_headings"] else article_with_outline.root.section_name
                    section_output_dict["parent_section_name"] = parent_section_name
                    section_output_dict_collection.append(section_output_dict)
                except Exception as e:
                     logging.error(f"Error processing section generation future: {e}", exc_info=True)
        ...
        for section_output_dict in section_output_dict_collection:
            ...
            if article.find_section(article.root, parent_section_name):
                 ...
                 logging.warning(f"Parent section '{parent_section_name}' not found for section '{section_output_dict['section_name']}'. Skipping update.")
...
class ConvToSection(dspy.Module):
    ...
    def forward(
        self, transcript: str, outline: str, section: str, collected_info: List[Information], topic: str, figure_data: Optional[List[Dict[str, str]]] = None):
        info = ""
        if collected_info: # Check if collected_info is not empty
            for idx, storm_info in enumerate(collected_info):
                ...
                snippets_text = "\n".join(storm_info.snippets) if storm_info.snippets else "No snippets available."
                info += f"[{idx + 1}]\n" + snippets_text + "\n\n"
        ...
        if figure_data:
            try:
                figure_data_str = json.dumps(figure_data, indent=2) # Convert list of dicts to JSON string
            except TypeError:
                figure_data_str = str(figure_data) # Fallback to simple string conversion if JSON fails

        with dspy.settings.context(lm=self.engine):
            section_content = ArticleTextProcessing.clean_up_section(
                self.write_section(transcript=transcript, info=info, section=section, outline=outline, topic=topic, figure_data=figure_data_str).output # Pass figure_data_str
            )
...
class WriteSection(dspy.Signature):
    ...
    transcript = dspy.InputField(prefix="transcript(or 'N/A' if not available):\n", format=str)
    ...
    figure_data = dspy.InputField(prefix="Available figures (list of dictionaries with 'image_path', 'figure_name', 'caption', or 'N/A' if no figures):\n", format=str, optional=True)
    output = dspy.OutputField(
        prefix="When writing, use the correct inline citations (e.g., [1][2] for web sources; [transcript 1][transcript 2] for provided transcripts; [paper 1][paper 2] for provided papers) to write the section:\n",
        format=str,
    )
...
class ConvSimulator(dspy.Module):
    ...
    def forward(self, transcript: str, persona: str, ground_truth_url: str, callback_handler: BaseCallbackHandler, topic: Optional[str] = None, old_outline: Optional[str] = None):
        dlg_history: List[DialogueTurn] = []

        # Add an initial turn to gather data and figures related to the topic
        initial_question = f"Can you find any data and quantitative information about {topic}?"
        expert_output = self.topic_expert(transcript=transcript, question=initial_question, ground_truth_url=ground_truth_url, old_outline=old_outline)
        initial_turn = DialogueTurn(agent_utterance=expert_output.answer, user_utterance=initial_question, search_queries=expert_output.queries, search_results=expert_output.searched_results)
        dlg_history.append(initial_turn)
        callback_handler.on_dialogue_turn_end(dlg_turn=initial_turn)

        for _ in range(self.max_turn):
            user_utterance = self.wiki_writer(transcript=transcript, persona=persona, dialogue_turns=dlg_history, topic=topic, old_outline=old_outline).question
            if user_utterance == "":
                logging.error("Simulated writer utterance is empty.")
                break
            if user_utterance.startswith("Thank you so much for your help!"):
                break
            expert_output = self.topic_expert(transcript=transcript, question=user_utterance, ground_truth_url=ground_truth_url, old_outline=old_outline)
            dlg_turn = DialogueTurn(agent_utterance=expert_output.answer, user_utterance=user_utterance, search_queries=expert_output.queries, search_results=expert_output.searched_results)
...
class WikiWriter(dspy.Module):
    ...
    def forward(self, transcript: str, persona: str, dialogue_turns: List[DialogueTurn], draft_page=None, topic: Optional[str] = None, old_outline: Optional[str] = None):
        conv = []
        ...
        with dspy.settings.context(lm=self.engine):
            if persona is not None and len(persona.strip()) > 0:
                question = self.ask_question_with_persona(transcript=transcript, persona=persona, conv=conv, topic=topic, old_outline=old_outline).question
            else:
                question = self.ask_question(transcript=transcript, conv=conv, topic=topic, old_outline=old_outline).question
        return dspy.Prediction(question=question)

class AskQuestion(dspy.Signature):
    __doc__ = prompts.AskQuestion_docstring
    transcript = dspy.InputField(prefix="The presentation transcript (or 'N/A' if not available): ", format=str)
    conv = dspy.InputField(prefix="Conversation history:\n", format=str)
    topic = dspy.InputField(prefix="Topic to focus on: ", format=str)
    old_outline = dspy.InputField(prefix="Existing outline to help guide questions: ", format=str, required=False)
...
class AskQuestionWithPersona(dspy.Signature):
    ...
    transcript = dspy.InputField(prefix="The presentation transcript (or 'N/A' if not available): ", format=str)
    persona = dspy.InputField(prefix="Your persona besides being a report writer: ", format=str)
    conv = dspy.InputField(prefix="Conversation history:\n", format=str)
    topic = dspy.InputField(prefix="Topic to focus on: ", format=str)
    old_outline = dspy.InputField(prefix="Existing outline to help guide questions: ", format=str, required=False)
    question = dspy.OutputField(format=str)
...
class QuestionToQuery(dspy.Signature):
    ...
    old_outline = dspy.InputField(prefix="Existing outline to help guide search: ", format=str, required=False)
...
class AnswerQuestion(dspy.Signature):
    ...
    old_outline = dspy.InputField(prefix="Existing outline to help guide the answer: ", format=str, required=False)
...
class TopicExpert(dspy.Module):
    ...
    def forward(self, transcript: str, question: str, ground_truth_url: str, old_outline: Optional[str] = None):
        with dspy.settings.context(lm=self.engine, show_guidelines=False):
            queries = self.generate_queries(transcript=transcript, question=question, old_outline=old_outline).queries
            queries = [q.replace("-", "").strip().strip('"').strip('"').strip() for q in queries.split("\n")]
            queries = queries[: self.max_search_queries]
            searched_results: List[Information] = self.retriever.retrieve(list(set(queries)), exclude_urls=[ground_truth_url])
            if len(searched_results) > 0:
                ...
                try:
                    answer = self.answer_question(transcript=transcript, conv=question, info=info, old_outline=old_outline).answer
...
class StormKnowledgeCurationModule(KnowledgeCurationModule):
    def __init__(self, retriever: Retriever, persona_generator: Optional[StormPersonaGenerator], conv_simulator_lm: dspy.LM, question_asker_lm: dspy.LM, max_search_queries_per_turn: int, search_top_k: int, max_conv_turn: int, max_thread_num: int):
        ...
        self.retriever = retriever
        self.conv_simulator = ConvSimulator(topic_expert_engine=conv_simulator_lm, question_asker_engine=question_asker_lm, retriever=retriever, max_search_queries_per_turn=max_search_queries_per_turn, search_top_k=search_top_k, max_turn=max_conv_turn)

    def _get_considered_personas(self, transcript: str, max_num_persona, topic: Optional[str] = None, old_outline: Optional[str] = None) -> List[str]:
        return self.persona_generator.generate_persona(transcript=transcript, max_num_persona=max_num_persona, topic=topic, old_outline=old_outline)

    def _run_conversation(self, conv_simulator, transcript, ground_truth_url, considered_personas, callback_handler: BaseCallbackHandler, topic: Optional[str] = None, old_outline: Optional[str] = None) -> List[Tuple[str, List[DialogueTurn]]]:
        conversations = []
        def run_conv(persona):
            return conv_simulator(transcript=transcript, ground_truth_url=ground_truth_url, persona=persona, callback_handler=callback_handler, topic=topic, old_outline=old_outline)
    ...
    def research(self, transcript: str, ground_truth_url: str, callback_handler: BaseCallbackHandler, max_perspective: int = 0, disable_perspective: bool = True, return_conversation_log=False, topic: Optional[str] = None, old_outline: Optional[str] = None) -> Union[StormInformationTable, Tuple[StormInformationTable, Dict]]:
        callback_handler.on_identify_perspective_start()
        considered_personas = []
        if disable_perspective:
            considered_personas = [""]
        else:
            considered_personas = self._get_considered_personas(transcript=transcript, max_num_persona=max_perspective, topic=topic, old_outline=old_outline)
        ...
        conversations = self._run_conversation(conv_simulator=self.conv_simulator, transcript=transcript, ground_truth_url=ground_truth_url, considered_personas=considered_personas, callback_handler=callback_handler, topic=topic, old_outline=old_outline)

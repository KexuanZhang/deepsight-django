...
class StormOutlineGenerationModule(OutlineGenerationModule):
    ...
    def generate_outline(self, transcript: str, information_table: StormInformationTable, old_outline: Optional[StormArticle] = None, callback_handler: BaseCallbackHandler = None, return_draft_outline=False, topic: Optional[str] = None, output_dir: Optional[str] = None) -> Union[StormArticle, Tuple[StormArticle, StormArticle]]:
        ...
        if old_outline is not None:
            outline_list = old_outline.get_outline_as_list(add_hashtags=True, include_root=False)
            old_outline_str = '\n'.join(outline_list)
        result = self.write_outline(transcript=transcript, dlg_history=concatenated_dialogue_turns, old_outline=old_outline_str, callback_handler=callback_handler, topic=topic, output_dir=output_dir)
        
        # Post-process the outline to filter headings up to level 3
        outline_lines = result.outline.split('\n')
        filtered_lines = [line for line in outline_lines if re.match(r'^#{1,3} .+$', line.strip())]
        complete_outline = '\n'.join(filtered_lines)
        result.outline = complete_outline
        
        article_with_outline_only = StormArticle.from_outline_str(topic="StormReport", outline_str=result.outline)
        article_with_draft_outline_only = StormArticle.from_outline_str(topic="StormReport", outline_str=result.old_outline)
...
class WriteOutline(dspy.Module):
    ...
    def forward(self, transcript: str, dlg_history, old_outline: Optional[str] = None, callback_handler: BaseCallbackHandler = None, topic: Optional[str] = None, output_dir: Optional[str] = None):
        trimmed_dlg_history = [turn for turn in dlg_history if "topic you" not in turn.agent_utterance.lower() and "topic you" not in turn.user_utterance.lower()]
        conv = "\n".join([f"Report Writer: {turn.user_utterance}\nExpert: {turn.agent_utterance}" for turn in trimmed_dlg_history])
        ...
        with dspy.settings.context(lm=self.engine):
            if old_outline is None:
                ...
                old_outline = ArticleTextProcessing.clean_up_outline(self.draft_page_outline(transcript=transcript, topic=topic).outline)
                if callback_handler:
                ...
                if self.outline_rater:
                    ...
                    if output_dir:
                        outline_score_path = os.path.join(output_dir, "outline_score.json")
                        if os.path.exists(outline_score_path):
                            try:
                                with open(outline_score_path, 'r', encoding='utf-8') as f:
                                    outline_score_json = json.dumps(json.load(f), indent=2)
...
class WritePageOutline(dspy.Signature):
    ...
    outline = dspy.OutputField(prefix="Write the report outline based on the topic:\n", format=str)
...
class NaiveOutlineGen(dspy.Module):
    ...
    def forward(self, transcript: str):
        outline = self.write_outline(transcript=transcript).outline
...
class WritePageOutlineFromConv(dspy.Signature):
    ...
    topic = dspy.InputField(prefix="Topic to focus on:\n", format=str)
    outline_score = dspy.InputField(prefix="Outline section ratings (higher scores indicate more important sections):\n", format=str)
    outline = dspy.OutputField(prefix="Write the tech report outline based on the topic (Use '#' for level 1 headings, '##' for level 2, '###' for level 3, and make sure every heading ends with a period):\n", format=str)
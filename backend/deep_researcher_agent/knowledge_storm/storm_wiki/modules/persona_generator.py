...
class FindRelatedTopic(dspy.Signature):
    ...
    old_outline = dspy.InputField(prefix="Existing outline to guide the search: ", format=str, required=False)
...
class GenPersona(dspy.Signature):
    ...
    topic = dspy.InputField(prefix="Topic to focus on: ", format=str, required=False)
    old_outline = dspy.InputField(prefix="Existing outline to guide persona creation: ", format=str, required=False)
...
class CreateWriterWithPersona(dspy.Module):
    ...
    def forward(self, transcript: str, topic: Optional[str] = None, old_outline: Optional[str] = None, draft=None):
        with dspy.settings.context(lm=self.engine):
            related_topics = self.find_related_topic(transcript=transcript, topic=topic, old_outline=old_outline).related_topics
            gen_persona_output = self.gen_persona(transcript=transcript, topic=topic, old_outline=old_outline).personas
        personas = [match.group(1) for s in gen_persona_output.split("\n") if (match := re.search(r"\d+\.\s*(.*)", s))]
...
class StormPersonaGenerator:
    ...
    def generate_persona(self, transcript: str, max_num_persona: int = 3, topic: Optional[str] = None, old_outline: Optional[str] = None) -> List[str]:
        """
        ...
        """
        personas = self.create_writer_with_persona(transcript=transcript, topic=topic, old_outline=old_outline)

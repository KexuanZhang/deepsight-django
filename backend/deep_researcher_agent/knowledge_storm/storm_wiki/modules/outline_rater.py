...
def parse_outline_sections(outline_text: str) -> Dict[str, str]:
    ...
    has_single_hash = any(line.strip().startswith('# ') and not line.strip().startswith('## ') for line in lines)
    ...
    for i, line in enumerate(lines):
        ...
        if line.strip().startswith(l1_pattern) and not line.strip().startswith(l1_pattern + '#'):
...
def rate_outline(client, old_outline: str, conv_history: Optional[str] = None, transcript: Optional[str] = None, paper_content: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        paper_content: Optional paper content
    ...
    try:
        ...
        if paper_content:
            context_parts.append(f"<paper_content>\n{paper_content}\n</paper_content>")
        if conv_history:
...
def rate_l2_headings(client, l1_section: str, l1_heading: str, conv_history: Optional[str] = None, transcript: Optional[str] = None, paper_content: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(f"Error parsing API response for L2 headings rating: {e}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
                return {"error": f"Error parsing API response: {str(e)}"}
...
def reassemble_outline(
    ...
    if heading_mapping:
        ...
        sorted_l1_headings = [heading_mapping[h] for h in sorted_rated_headings if h in heading_mapping]
    else:
        ...
        for heading in l1_sections.keys():
            if heading.startswith("#"):
                ...
                if match:
                    ...
                    logger.info(f"Found L1 heading format: '{original_level_marker}' from '{heading}'")
                    break
        
        logger.info(f"Using original L1 heading level marker: '{original_level_marker}'")
        ...
        for heading in list(l1_ratings.keys()):
            ...
            if heading != correct_heading:
                logger.warning(f"Heading format mismatch: '{heading}' vs correct format '{correct_heading}'")
    ...
    for l1_heading in sorted_l1_headings:
        ...
        if l1_heading not in l1_sections:
            logger.warning(f"L1 heading '{l1_heading}' from ratings not found in original outline sections")
            continue
        
        l1_content = l1_sections[l1_heading]
        
        # Check if we have L2 ratings for this L1 section
        if l2_ratings_by_l1 and l1_heading in l2_ratings_by_l1 and l2_ratings_by_l1[l1_heading]:
...
class OutlineRater:
    ...
    def _configure_openai_client(self):
        ...
        try:
            ...
            workspace_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(current_script_path))))
            ...
            if os.path.exists(secrets_path):
                ...
                if api_key:
                    ...
                    logger.warning("OPENAI_API_KEY not found in secrets.toml. Falling back to environment variables.")
            else:
                logger.warning(f"secrets.toml not found at expected path: {secrets_path}. Falling back to environment variables.")
        except Exception as e:
        ...
        try:
            if api_type == "azure":
                ...
                if not api_base or not api_version:
                    logger.warning("AZURE_API_BASE or AZURE_API_VERSION not found for Azure configuration.")
                    return None
    ...
    def rate_and_reassemble_outline(
        ...
        try:
            ...
            l1_ratings = rate_outline(self.client, outline, conv_history, transcript, paper_content)
            ...
            if not l1_sections and outline:
                logger.warning(f"No L1 sections found but outline exists. Outline content: {outline[:50]}...")
            ...
            for original_heading in l1_sections.keys():
                if original_heading in l1_ratings:
                    ...
                    original_clean = re.sub(r'^#+\s', '', original_heading).strip().lower()
                    best_match = None
                    best_score = 0
                    
                    for rated_heading in l1_ratings.keys():
                        rated_clean = re.sub(r'^#+\s', '', rated_heading).strip().lower()
                        # Simple fuzzy matching: check if one is contained in the other
                        if rated_clean in original_clean or original_clean in rated_clean:
                    ...
                    if best_match and best_score > 0.3:  # Threshold for matching
                        ...
                        logger.info(f"Fuzzy mapped rated heading '{best_match}' to original '{original_heading}'")
                    else:
                        logger.warning(f"No good match found for original heading '{original_heading}'")
            ...
            if self.client:
                for original_heading in l1_sections:
                    ...
                    if l2_headings:  # Only rate if there are L2 headings
                        l2_ratings = rate_l2_headings(self.client, section_content, original_heading, conv_history, transcript, paper_content)
                        if "error" not in l2_ratings and l2_ratings:  # Check if not empty
                            l2_ratings_by_l1[original_heading] = l2_ratings
                        else:
                            if "error" in l2_ratings:
                                logger.error(f"Failed to get L2 ratings for L1 section '{original_heading}': {l2_ratings.get('error')}")
                            else:
                                logger.warning(f"L2 ratings empty for L1 section '{original_heading}'")
                    else:
                        logger.info(f"No L2 headings found in section '{original_heading}', skipping L2 rating")
            else:
            ...
            reassembled_outline = reassemble_outline(outline, l1_ratings, l2_ratings_by_l1, heading_mapping)
            
            # Save the reassembled outline if output directory is provided
            if self.output_dir:
                try:
                    reordered_path = os.path.join(self.output_dir, "reordered_outline.txt")
...
def main():
    parser = argparse.ArgumentParser(description="Rate an outline based on direct input or file.")
    parser.add_argument("--outline", required=True, help="Path to file containing the outline to be rated")
    parser.add_argument("--conv_history", help="Path to file containing conversation history (optional)")
    parser.add_argument("--transcript", help="Path to file containing transcript (optional)")
    parser.add_argument("--paper", help="Path to file containing paper content (optional)")
    parser.add_argument("--output", help="Path to output JSON file (optional)")
    parser.add_argument("--stage", choices=["A", "B", "reassemble"], default="A", help="Stage A: Rate L1 headings, Stage B: Rate L2 headings within a specific L1 section, reassemble: Reorder outline based on ratings")
    parser.add_argument("--l1_heading", help="L1 heading to analyze in Stage B (required for Stage B)")
    parser.add_argument("--l1_ratings", help="Path to JSON file with L1 ratings (required for reassemble stage)")
    parser.add_argument("--l2_ratings", help="Path to JSON file with L2 ratings (optional for reassemble stage)")
    parser.add_argument("--reassembled_output", help="Path to save reassembled outline (required for reassemble stage)")
    ...
    transcript = read_file_content(args.transcript) if args.transcript else None
    paper_content = read_file_content(args.paper) if args.paper else None
    ...
    if args.stage == "A":
        ...
        rating_result = rate_outline(rater.client, outline_content, conv_history, transcript, paper_content)
        ...
        if not args.l1_heading:
            ...
            logger.info("Stage B: No specific L1 heading provided. Processing all L1 sections...")
            ...
            for l1_heading, section_content in l1_sections.items():
                l2_ratings = rate_l2_headings(rater.client, section_content, l1_heading, conv_history, transcript, paper_content)
            ...
            if not matching_heading:
                logger.error(f"L1 heading '{args.l1_heading}' not found in the outline.")
                return
            ...
            rating_result = rate_l2_headings(rater.client, section_content, matching_heading, conv_history, transcript, paper_content)
        ...
        if not args.reassembled_output:
            logger.error("Missing --reassembled_output parameter for saving the reassembled outline.")
            return
        ...
        reassembled_outline = reassemble_outline(outline_content, l1_ratings, l2_ratings_by_l1, None)

...
class BaseQueueService(BaseService):
    ...
    def store_job_metadata(self, job_id: str, metadata: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.redis_client.hset(
                f"{self.job_metadata_key}:{job_id}",
                mapping=redis_data
            )
    ...
    def update_job_progress(self, job_id: str, progress: str, status: Optional[str] = None, percentage: Optional[int] = None):
        ...
        try:
            ...
            if status:
                self._publish_status_update(job_id, {
                    "status": status,
                    "progress": progress,
            ...
            self.log_operation("update_progress", f"job_id={job_id}, progress={progress}")
    ...
    def update_job_error(self, job_id: str, error: str):
        ...
        try:
            ...
            self.log_operation("update_error", f"job_id={job_id}, error={error[:100]}", "error")
    ...
    def _publish_status_update(self, job_id: str, status_data: Dict[str, Any]):
        ...
        try:
            ...
            self.log_operation("publish_status", f"job_id={job_id}, channel={channel_name}")
        except Exception as e:
            self.log_operation("publish_status_error", f"job_id={job_id}, error={str(e)}", "error")

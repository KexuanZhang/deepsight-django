...
class NotebooksConfig:
    ...
    def __init__(self):
        ...
        self.ENABLE_MAGIC_VALIDATION = getattr(
            django_settings, "NOTEBOOKS_ENABLE_MAGIC_VALIDATION", True
        )
...
class DeepSightStorageConfig:
    ...
    def get_knowledge_base_path(
        self, user_id: int, year_month: str, file_id: str
    ) -> Path:
        """Get the knowledge base path for a specific file."""
        return (
            self.get_user_base_path(user_id)
            / "knowledge_base_item"
            / year_month
            / f"f_{file_id}"
        )
    ...
    def get_report_path(self, user_id: int, year_month: str, report_id: str) -> Path:
        ...
        return (
            self.get_user_base_path(user_id) / "report" / year_month / f"r_{report_id}"
        )

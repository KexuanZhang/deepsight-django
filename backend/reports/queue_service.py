...
class ReportQueueService:
    ...
    def add_report_job(self, report: Report) -> str:
        ...
        try:
            ...
            logger.info(f"Report generation job {report.job_id} queued successfully for report {report.id}")
            return report.job_id
            
        except Exception as e:
            logger.error(f"Error adding report job for report {report.id}: {e}")
            report.update_status(Report.STATUS_FAILED, error=f"Failed to queue job: {str(e)}")
            raise
    ...
    def update_job_error(self, job_id: str, error: str):
        ...
        try:
            report = Report.objects.get(job_id=job_id)
            report.update_status(Report.STATUS_FAILED, progress=f"Job failed: {error}", error=error)

...
class ReportCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_max_conv_turn(self, value):
        ...
        if not 1 <= value <= 10:
            raise serializers.ValidationError("Max conversation turns must be between 1 and 10")
        return value
    
    def validate_max_perspective(self, value):
        """Validate max_perspective is in valid range."""
        if not 1 <= value <= 10:
            raise serializers.ValidationError("Max perspective must be between 1 and 10")
        return value
    ...
    def validate_initial_retrieval_k(self, value):
        ...
        if not 50 <= value <= 500:
            raise serializers.ValidationError("Initial retrieval K must be between 50 and 500")
        return value
    
    def validate_final_context_k(self, value):
        """Validate final_context_k is in valid range."""
        if not 10 <= value <= 100:
            raise serializers.ValidationError("Final context K must be between 10 and 100")
        return value
    
    def validate_reranker_threshold(self, value):
        """Validate reranker_threshold is in valid range."""
        if not 0.0 <= value <= 1.0:
            raise serializers.ValidationError("Reranker threshold must be between 0.0 and 1.0")
        return value
    
    def validate_max_thread_num(self, value):
        """Validate max_thread_num is in valid range."""
        if not 1 <= value <= 20:
            raise serializers.ValidationError("Max thread number must be between 1 and 20")
        return value
...
class ReportGenerationRequestSerializer(serializers.Serializer):
    ...
    prompt_type = serializers.ChoiceField(choices=Report.PROMPT_TYPE_CHOICES, default=Report.PROMPT_TYPE_GENERAL)
    ...
    initial_retrieval_k = serializers.IntegerField(default=150, min_value=50, max_value=500)
    final_context_k = serializers.IntegerField(default=20, min_value=10, max_value=100)
    reranker_threshold = serializers.FloatField(default=0.5, min_value=0.0, max_value=1.0)
    max_thread_num = serializers.IntegerField(default=10, min_value=1, max_value=20)
    
    # Optional parameters
    time_range = serializers.ChoiceField(choices=Report.TIME_RANGE_CHOICES, required=False, allow_null=True, allow_blank=True)
    include_domains = serializers.BooleanField(default=False)
    skip_rewrite_outline = serializers.BooleanField(default=False)
    domain_list = serializers.ListField(child=serializers.CharField(), required=False, default=list)

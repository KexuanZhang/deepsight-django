...
export const getVideoCompatibility = (format) => {
  const formatLower = format?.toLowerCase();
  return VIDEO_COMPATIBILITY[formatLower] || {
    supported: 'unknown',
    reason: 'Browser support for this format may vary'
  };
};

export const getAudioMimeType = (format) => {
  return AUDIO_MIME_TYPES[format?.toLowerCase()] || `audio/${format?.toLowerCase()}`;
};

export const getVideoMimeType = (format) => {
  return VIDEO_MIME_TYPES[format?.toLowerCase()] || `video/${format?.toLowerCase()}`;
};
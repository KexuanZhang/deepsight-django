...
│   └── 20250711_143028_c3d4e5f6789abcde_ah6g4c3d.docx
├── reports/                                      # Generated reports
│   ├── 20250711_143030_d4e5f6789abcdef0_bi7h5d4e.pdf
│   └── 20250711_143035_e5f6789abcdef012_cj8i6e5f.pdf
├── podcasts/                                     # Generated podcasts
│   ├── 20250711_143040_f6789abcdef01234_dk9j7f6g.mp3
│   └── 20250711_143045_123456789abcdef0_el0k8g7h.wav
├── kb-images/                                    # Extracted images
│   ├── 20250711_143050_23456789abcdef01_fm1l9h8i.png
│   └── 20250711_143055_3456789abcdef012_gn2m0i9j.jpg
└── temp/                                         # Temporary uploads
    └── 20250711_143100_456789abcdef0123_ho3n1j0k.tmp

# Object Key Pattern: {prefix}/{timestamp}_{content_hash}_{uuid}{extension}
# - timestamp: YYYYMMDD_HHMMSS (enables time-based queries)
# - content_hash: First 16 chars of SHA256 (enables deduplication)
# - uuid: 8-char UUID (prevents collisions)
# - extension: Original file extension (preserves file type)
```
...
### **Database Migration Strategy for MinIO-Native Approach**

#### **Phase 1: Schema Preparation**
```python
# Django migration to add MinIO-native fields
...
class Migration(migrations.Migration):
    ...
    operations = [
        # Add MinIO-native fields to KnowledgeBaseItem
        migrations.AddField(
            model_name='knowledgebaseitem',
            name='storage_uuid',
            field=models.UUIDField(default=uuid.uuid4, unique=True, db_index=True),
        ...
        # Add MinIO-native fields to Report
        migrations.AddField(
            model_name='report',
            name='storage_uuid',
            field=models.UUIDField(default=uuid.uuid4, unique=True, db_index=True),
        ...
        # Add MinIO-native fields to PodcastJob
        migrations.AddField(
...
#### **Phase 2: Data Migration to MinIO-Native**
```python
# Django management command for MinIO-native migration
from django.core.management.base import BaseCommand
from django.db import transaction
from notebooks.models import KnowledgeBaseItem, Report, PodcastJob
from notebooks.utils.minio_backend import MinIOBackend

class Command(BaseCommand):
    help = 'Migrate existing files to MinIO-native object key approach'
    ...
    def handle(self, *args, **options):
        ...
        # Migrate KnowledgeBaseItems
        kb_items = KnowledgeBaseItem.objects.filter(
            original_file_object_key__isnull=True
        )
        
        for item in kb_items.iterator(chunk_size=batch_size):
            try:
                with transaction.atomic():
                    self._migrate_kb_item(item, minio_backend, dry_run)
            except Exception as e:
                self.stdout.write(f'Error migrating KB item {item.id}: {str(e)}')
        
        self.stdout.write(self.style.SUCCESS('Migration completed'))
    
    def _migrate_kb_item(self, item, minio_backend, dry_run):
        """Migrate a KnowledgeBaseItem to MinIO-native approach"""
        
        # Read existing file content
        file_content = None
        if hasattr(item, 'original_file') and item.original_file:
            try:
                with item.original_file.open('rb') as f:
                    file_content = f.read()
            except (FileNotFoundError, OSError):
        ...
        if dry_run:
            self.stdout.write(f'Would migrate KB item {item.id}')
            return
        
        # Generate new MinIO object key
        original_filename = item.metadata.get('original_filename', 'migrated_file')
        object_key = minio_backend.save_file_with_auto_key(
            ...
            metadata={
                ...
                'migration_source': 'django_migration'
            }
        )
        
        # Update database with MinIO object key
        item.original_file_object_key = object_key
        item.file_metadata = {
            'filename': original_filename,
        ...
        self.stdout.write(f'Migrated KB item {item.id} -> {object_key}')
```

#### **Phase 3: Rollback Strategy for MinIO-Native**
```python
# Rollback from MinIO-native to FileField approach
class Command(BaseCommand):
    help = 'Rollback from MinIO-native to FileField storage'
    
    def handle(self, *args, **options):
        minio_backend = MinIOBackend()
        
        # Get all items with MinIO object keys
        kb_items = KnowledgeBaseItem.objects.filter(
            original_file_object_key__isnull=False
        )
        
        for item in kb_items:
            try:
                # Download from MinIO
                file_content = minio_backend.get_file_content(item.original_file_object_key)
                
                # Recreate local file structure
                filename = item.file_metadata.get('filename', 'restored_file')
                local_path = f"restored/{item.user_id}/{item.id}/{filename}"
                
                # Save to Django FileField
                from django.core.files.base import ContentFile
                item.original_file.save(local_path, ContentFile(file_content))
                
                # Clear MinIO fields
                item.original_file_object_key = None
                item.file_metadata = {}
                item.save()
                
                self.stdout.write(f'Rolled back item {item.id}')
                
            except Exception as e:
                self.stdout.write(f'Error rolling back item {item.id}: {str(e)}')
```

### **Key Benefits of MinIO-Native Database Design**

1. **Zero Path Management**: No upload_to functions or custom path logic needed
2. **MinIO-Optimized**: Object keys designed for optimal MinIO performance
3. **Auto-Deduplication**: Content-based hashing prevents duplicate storage
4. **Direct Object Access**: Database indexes on MinIO object keys for fast lookups
5. **Rich Metadata**: File information stored in database JSONField
6. **User Isolation**: Built-in user-based access control via object metadata
7. **Efficient Queries**: Optimized indexes for MinIO operations
8. **Cloud-Native**: Follows S3/MinIO best practices for object storage
9. **Scalable**: Flat object structure enables unlimited horizontal scaling
10. **Future-Proof**: No filesystem limitations, works with any object storage

    ...
    def validate_file(self, file: UploadFile) -> Dict[str, any]:
        ...
        if hasattr(file, 'size') and file.size:
            if file.size > self.max_file_size:
                errors.append(f"File size {file.size / (1024*1024):.1f}MB exceeds maximum allowed size of {self.max_file_size / (1024*1024):.0f}MB")
        ...
        if content_type and expected_content_type and not content_type.startswith(expected_content_type.split('/')[0]):
            warnings.append(f"MIME type {content_type} doesn't match file extension {file_extension}")
    ...
    def validate_file_content(self, file_path: str) -> Dict[str, any]:
        ...
        try:
            ...
            if expected_mime and not mime_type.startswith(expected_mime.split('/')[0]):
                warnings.append(f"File content MIME type {mime_type} doesn't match extension {file_extension}")
            
            # Check for potentially malicious files
            dangerous_types = ['application/x-executable', 'application/x-dosexec', 'application/javascript']
            if mime_type in dangerous_types:
                errors.append(f"File type {mime_type} is not allowed for security reasons")
